        ...
        public async Task<IActionResult> GetGlobalConfigurationStyles()
        {
            try
            {
                ...
                using (DbConnection dbConnection = new DbConnection())
                {
                    ...
                    {
                        globalConfigurationStyleList = await Task.FromResult(connection.Query<GlobalConfigurationStyle>("spGetGlobalConfigurationStyles", null, commandType: CommandType.StoredProcedure).ToList());
                    }
                }

                if (globalConfigurationStyleList.Count == 0)
                {
                    return new JsonResult(new CustomResponse<List<GlobalConfigurationStyle>>() { StatusCode = (int)HttpStatusCodesAndMessages.HttpStatus.NotFound, Result = false, Message = string.Format(SystemMessages.msgDataNotExists, "Style") });
                }
                    ...
                    using (DbConnection dbConnection = new DbConnection())
                    {
                        using (var connection = dbConnection.Connection)
                        {
                            foreach (var item in globalConfigurationStyles)
                            {
                                ...
                                result = await Task.FromResult(connection.Execute("spUpdateGlobalConfigurationStyles", parameters, commandType: CommandType.StoredProcedure));
                            }
                        }
                        if (result == 1)
                        {
                            return new JsonResult(new CustomResponse<string>() { StatusCode = (int)HttpStatusCodesAndMessages.HttpStatus.Success, Result = true, Message = string.Format(SystemMessages.msgDataUpdatedSuccessfully, "GlobalConfiguration Styles") });
                        }
